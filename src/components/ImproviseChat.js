...
const ImproviseChat = ({ taskId }) => {
    ...
    const [improviseCommand, setImproviseCommands] = useState("I want to learn picking and strumming in detail. ");
    const [improvisingData, setImprovisingData] = useState();
    const fetchDescription = async () => {
        setIsLoading(true);
        try {
            const res = await askFromGemini(`Tell me a brief idea in less than 100 words on: ${topicName}`);
            setMiniDescription(res?.response?.text() || "Failed to fetch description.");
        ...
        <div className="w-full md:w-2/5 p-4 flex justify-center items-center border-l border-gray-700 bg-gradient-to-br from-gray-900 to-gray-800 min-h-screen">
            <div className="w-full max-w-md space-y-8 text-center text-white">
                ...
                <button
                    ...
                    className={`w-full ${isFetched ? "bg-gradient-to-r from-green-500 to-teal-600" : "bg-gradient-to-r from-blue-500 to-purple-600"
