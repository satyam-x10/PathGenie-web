...
export async function GET(req) {
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Please provide a topicID or chainId." }),
      {
        status: 400,
        headers: { "Content-Type": "application/json" },
      },
...
export async function POST(req) {
  try {
    ...
    const { taskID, title, resourceId, description, subtopics, parentTopic } =
      body;

    if (!taskID || !title || !resourceId) {
      return new Response(
        JSON.stringify({
          message: "taskID, title, and resourceId are required",
        }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({ message: "Topic created", newTopic }),
      {
        status: 201,
        headers: { "Content-Type": "application/json" },
      },
...
export async function PUT(req) {
  try {
    ...
    if (!updatedTopic) {
      return new Response(
        JSON.stringify({ message: "Topic not found or update failed" }),
        {
          status: 404,
          headers: { "Content-Type": "application/json" },
        },
    ...
    return new Response(
      JSON.stringify({ message: "Topic updated", updatedTopic }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
      },
...
export async function DELETE(req) {
  try {
    ...
    if (!id) {
      return new Response(
        JSON.stringify({ message: "Topic ID query parameter required" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
    ...
    if (!deletedTopic) {
      return new Response(
        JSON.stringify({ message: "Topic not found or delete failed" }),
        {
          status: 404,
          headers: { "Content-Type": "application/json" },
        },
    ...
    return new Response(
      JSON.stringify({ message: "Topic deleted", deletedTopic }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
      },
